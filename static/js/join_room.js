...
socket.on('join_by_private_room_code_response', function(json) {
    if (json.join_status === 1) {
        document.getElementById('join_room_message').innerText = 
            "Joining room!";
    } else if (json.join_status === 0) {
        document.getElementById('join_room_message').innerText = 
            "Oops ... it doesn't seem like that room exists.";
    } else if (json.join_status === -1) {
        document.getElementById('join_room_message').innerText = 
            "Oops ... it seems like the room is full.";
    }
});

socket.on('start_game_private_room', function() {
    document.getElementById('enter_room_code_view').style.display = 'none';
...
function setup() {
    ...
    document.getElementById('join_room_button').onclick = () => {
        socket.emit('join_by_private_room_code', {'room_code': document.getElementById('room_code_input').value})
    }
}
